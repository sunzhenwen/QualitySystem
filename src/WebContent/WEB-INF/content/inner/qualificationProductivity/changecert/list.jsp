...
function querySpecialtyType(){
	...
	function query()
	{
		document.form1.action="<%=basepath%>/changeCertAction_queryPage.action";
	...
	function batchApplythrough(){   
		...
		if(confirm("您确定要审批选中的记录吗？(只能批量通过！！)")==true){
			document.form1.action="<%=basepath%>/changeCertAction_batchApplythrough.action";
			document.form1.submit();
		}
	}
</script>
				...
				<s:iterator status="i" value="currentPage.data">
					<tr>
						...
						<s:if test="checkNum != 1">
							<td align='center'><a
								href="changeCertAction_queryInfo.action?id=<s:property value="newQualId"/>">查看</a>
							</td>
						</s:if>
					</tr>
				</s:iterator>
