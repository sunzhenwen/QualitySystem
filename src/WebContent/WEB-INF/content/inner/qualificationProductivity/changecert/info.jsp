...
<script type="text/javascript">
	...
	function applythrough()
	{
		...
		if(nm.value != "")
		{
			if(confirm("请确认是否通过"))
			{
				document.form1.action="<%=basepath%>/changeCertAction_approvalSuccess.action?s="+Math.random();
	...
	function applyBack()
	{
		...
		if(nmv.value != "")
		{
			if(confirm("请确认是否不通过"))
			{
				document.form1.action="<%=basepath%>/changeCertAction_approvalFailed.action";
				document.form1.submit();
			}
			else
			{
	...
	function goBack()
	{
		document.form1.action="<%=basepath%>/changeCertAction_goBack.action";
		document.form1.submit();
	}
