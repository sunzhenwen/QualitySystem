...
function querySpecialtyType(){
	...
	function query()
	{
		document.form1.action="<%=basepath %>/twoApprovalAction_queryPage.action";
	...
	function batchApplythrough()
	{
		...
		if(confirm("您确定要审批选中的记录吗？")==true){
			document.form1.action="<%=basepath %>/twoApprovalAction_batchApplythrough.action";
			document.form1.submit();
		}
	}	
</script>
...
<table width="100%"  border="0" cellpadding="0" height="15%">
     <tr>
	    <td >	
	      <fieldset>
		  <legend  class="chaxuntj" >专业厅局审批</legend>	
          <p style="margin-bottom:10px; text-align:right;">
          <a href="#" onclick="query()"><span class="lbchx">查询</span></a> 
          <a href="#" onclick="batchApplythrough()"><span class="lbchx">批量审批</span></a> 
...
<table width = "100%" border  = "0"  cellpadding = "0"  cellspacing="0"   onmouseover="changeto()"  onmouseout="changeback()" >
	...
	<tr>
		...
		<s:if test="dateMark==0">
			<td style="color:#53B366"><strong><s:date name="deadDate" format="yyyy-MM-dd" />还剩<s:property value="differDay"/>天</strong></td>
		</s:if>
		<s:if test="dateMark==1">
			<td style="color:#EE9A00"><strong><s:date name="deadDate" format="yyyy-MM-dd" />还剩<s:property value="differDay"/>天</strong></td>
		</s:if>
		<s:if test="dateMark==2">
			<td style="color:#FF0000"><strong><s:date name="deadDate" format="yyyy-MM-dd" />已超期</strong></td>
		</s:if>
		...
		<td><input type="checkbox" name="ids" value="<s:property value="newQualId"/>" onclick="javascript:setChecked(form1);"/></td>
		<td align='center'> <a href="twoApprovalAction_approvalPage.action?newQualId=<s:property value="newQualId"/>">审批</a> </td>
