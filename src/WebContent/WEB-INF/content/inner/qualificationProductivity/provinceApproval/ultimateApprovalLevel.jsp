...
<script type="text/javascript">
	...
	function applythrough()
	{
		...
		if(nm.value != "")
		{
			if(confirm("请确认是否通过"))
			{
				document.form1.action="<%=basepath %>/provinceApprovalAction_applyUltimateThrough.action";
	...
	function applyBack()
	{
		...
		if(nmv.value != "")
		{
			if(confirm("请确认是否不通过"))
			{
				document.form1.action="<%=basepath %>/provinceApprovalAction_applyUltimateThroughBack.action";
				document.form1.submit();
			}
			else
			{
            ...
            <table class="sp_mtable" style="border-collapse:collapse;" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>
                    	...
                    	<input type="hidden" name="newQualification.twoApprovalStaff" value="<s:property value="newQualification.twoApprovalStaff"/>" />
                    	<input type="hidden" name="newQualification.twoApprovalStaffId" value="<s:property value="newQualification.twoApprovalStaffId"/>" />
                ...
                <tr>
                    <td>专业厅局审批结果：</td>
               		<td>
               			<s:if test="newQualification.oneResults==3">
               				<font color="green">审批通过</font>
               			</s:if>
               			<s:if test="newQualification.oneResults==4">
               				<font color="red">审批未通过</font>
               			</s:if>
               			<input type="hidden" name="newQualification.oneResults" value="<s:property value="newQualification.oneResults"/>" />
					</td>
					<td>专业厅局审批时间：</td>
               		<td>
               			<s:date name="newQualification.oneAppDate" format="yyyy-MM-dd" />
               			<input type="hidden" name="newQualification.oneAppDate" value="<s:date name="newQualification.oneAppDate" format="yyyy-MM-dd" />" />
               		</td>
                </tr>
                <tr style="height:60px; border:0">
                    <td>专业厅局审批建议：</td>
                    <td colspan="3">
                    	<textarea readonly name="newQualification.oneUltimateOpinion"><s:property value="newQualification.oneUltimateOpinion"/></textarea>
                    </td>
                </tr>
                <tr style="height:60px; border:0">
                    <td>审批建议：</td>
               		<td colspan="3"> <textarea id="appId" name="newQualification.ultimateOpinion"></textarea> </td>
                </tr>
            </table>
            <div class="sp_bton"> 
