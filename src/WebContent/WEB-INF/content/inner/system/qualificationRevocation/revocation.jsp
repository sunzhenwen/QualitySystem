...
<script type="text/javascript">
	...
	function revocation()
	{
		...
		if(appId.value != ""){
			if(confirm("确认打回该资质")){
				document.form1.action="<%=basepath%>/qualificationRevocationAction_revocation.action";
	...
	function goBack()
	{
		document.form1.action="<%=basepath %>/qualificationRevocationAction_goBack.action";
		document.form1.submit();
	}
         ...
         <form id="form1" name="form1" method="post">
            <table class="sp_mtable" style="border-collapse:collapse;" cellpadding="0" cellspacing="0">
               ...
               <tr>
						...
						<input type="hidden" name="qualificationRevocation.cretGrade" value="<s:property value="qualificationRevocation.cretGrade"/>" /></td>
						<td>组织机构代码或统一社会信用代码：</td>
						<td><s:property value="qualificationRevocation.corpCode" />
							<input type="hidden" name="qualificationRevocation.corpCode" value="<s:property value="qualificationRevocation.corpCode"/>" />
						</td>
					</tr>
					...
					<tr>
		                ...
		                <td>审批部门：</td>
		              	<td><s:property value="qualificationRevocation.approvaldeptname"/>
		              		...
		              		<input type="hidden" name="qualificationRevocation.approvaldept" value="<s:property value="qualificationRevocation.approvaldept"/>" />
		        	</td>
		        </tr>
		        <tr>
                    <td>审批时间：</td>
                ...
                <tr style="height:60px; border:0">
                    <td>打回原因：</td>
               		<td colspan="3"> <textarea id="appId" name="qualificationRevocation.revocationReason" class="required"></textarea> </td>
                </tr>
            </table>
            <div class="sp_bton"> 
            	<a href="#" onclick="revocation()"><span class="sp_first">打回</span></a>
        	    <a href="#" onclick="goBack()"><span class="sp_last">取消</span></a>
            </div>
        </form>
    	</div>
  </div>
