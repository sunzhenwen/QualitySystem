 ...
 *
 */
public class QueryWriteOffTypeDaoImpl extends BaseDao implements IQueryWriteOffTypeDao {
...
//					certificatePrint.setRecoverStaffName(rs.getString("recoverStaffName"));//回复人姓名
	...
	public void batchWriteOffApp(List<CertificatePrint> list) {
		try {
			...
			{
				pstmt = conn.prepareStatement("update TBCORPCERTINFO set WRITEOFFAPPDATE=?,"
						+ "WRITEOFFTYPE=? where RELATIONID=?");
				pstmt.setDate(1, new Date(new java.util.Date().getTime()));
				pstmt.setString(2, "1");
				pstmt.setString(3,list.get(i).getRelationId());
				pstmt.executeUpdate();
			}
	...
	public void baseWriteOffApp(List<CertificatePrint> list) {
		try {
			...
			for(int i=0;i<list.size();i++)
			{
				...
				pstmt = conn.prepareStatement("update TBCORPCERTINFO set WRITEOFFAPPDATE=?,"
						+ "WRITEOFFTYPE=? where CORPCERTID=?");
				pstmt.setDate(1, new Date(new java.util.Date().getTime()));
				pstmt.setString(2, "1");
				pstmt.setString(3,list.get(i).getCorpCertId());
				pstmt.executeUpdate();
			}
			
			conn.commit();
			this.batchWriteOffApp(list);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			try {
