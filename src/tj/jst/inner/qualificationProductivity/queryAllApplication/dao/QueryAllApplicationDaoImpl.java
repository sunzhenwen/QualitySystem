package com.tj.jst.inner.qualificationProductivity.queryAllApplication.dao;
 ...
 *
 */
public class QueryAllApplicationDaoImpl extends BaseDao implements IQueryAllApplicationDao{
		...
		try{
			if(rs!=null){
				while(rs.next()){
					...
					//初审审批结果	3:通过 4:不通过
					if(!rs.getString("twoApprovalDeptName").equals("无")){
					...
					if(rs.getString("QUALIFTYPE") != null){
						if(rs.getString("QUALIFTYPE").trim().equals("5")){
							...
							certifiedService.setUltimateResults("待外厅审批");
						}else if(rs.getString("QUALIFTYPE").trim().equals("1")){
							if(rs.getString("cretGrade").trim().equals("3")){
								certifiedService.setUltimateResults("待地市处理");
							}else{
								certifiedService.setUltimateResults("待建管处理");
							}
						}else if(rs.getString("QUALIFTYPE").trim().equals("a")){
							certifiedService.setUltimateResults("待大厅处理");
						}else if(rs.getString("QUALIFTYPE").trim().equals("b")){
							certifiedService.setUltimateResults("未通过");
						}
					}
